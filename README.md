# JEFFERSONREP1
### Repositorio de Computación Gráfica

Este repositorio contiene los ejercicios y tareas desarrollados en clase utilizando tecnologías web para graficación.

## 🚀 Tareas Realizadas

A continuación se presentan los archivos del proyecto:

* **[🔴 Dibujo de Círculo](circulo.html)** - Implementación de trazado de círculos.
* **[🟦 Dibujo de Cuadrado](cuadrado.html)** - Renderizado de formas cuadradas.
* **[🔺 Dibujo de Triángulo](triangulo.html)** - Representación de triángulos.
* **[📐 Formas Combinadas](cuadradotriangulo.html)** - Interacción entre cuadrado y triángulo.
* **[✍️ Tarea de Composición](cuadradotriangulotarea.html)** - Ejercicio integrador de formas.
* **[📊 Gráfico de Barras](graficobarras.html)


*Estudiante: Jefferson Luna Quispe*
